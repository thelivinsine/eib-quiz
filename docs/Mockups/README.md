...
which names every place it deliberately differs. As of 2026-09-23 those are:
...
mode-agnostic ("Resume"), and the heading is 28px
...
the three readouts are one `--fs-md` figure over one 13px/400 label (the ring's
percentage is `--fs-xl` since 2026-09-23), where the
...
`tools/make-logo-kit.mjs`) and is the app's header mark, favicon and icons. Its two
BUTTONS shipped the same day: every button takes their 8px corner (`--radius-ctl`,
7.8 measured on a 48px body) and Learn More's near-black label and slate edge. The
rest of the board did not: its icon library is stroked where the app ships solid
glyphs.
