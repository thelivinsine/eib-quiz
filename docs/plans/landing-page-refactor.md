...
>
> **Also reversed (2026-09-23, on request):** the invariant "No drop shadows in either
> theme" in the table below. Tiles carry `--shadow-rest` and lift onto `--shadow-lift`
> on hover (see CLAUDE.md's "TILES CARRY A SOFT SHADOW AND LIFT ON HOVER"), so the
> phases' "no shadow" notes describe the page as it was built, not as it is.
